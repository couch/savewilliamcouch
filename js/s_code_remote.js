/* San Jose - Remote Code - Dynamic - G.9 patch 1 */
//4-05-05 Redirect Referrer and Alias GetCGI
//2-23-05 one Cookie Fix non-secure code
//2-25-05 Update to G.7 Code
//2005-11-09 Update to G.9 Code Kanji fix
...
var s_linkDownloadFileTypes="zip,wav,mp3,mp4,doc,pdf,xls,rss,dmg,sit,pkg,exe"
var s_linkLeaveQueryString=false
var s_linkTrackVars="s_prop3,s_hier5,s_campaign"
...
function s_doPlugins() {
	...
	// Redirect Referrer
	s_vp_getCGI('tempVar1','ref')
	if(s_vp_getValue('tempVar1'))
		s_vp_getCGI('s_referrer','ref');
	// Redirect Alias
	s_vp_getCGI('tempVar2','alias')
	if(s_vp_getValue('tempVar2'))
		s_vp_getCGI('s_server','alias');
	
	// External Campaigns
	if(!s_campaign){s_vp_getCGI('s_campaign','cid');}
	// Internal Campaigns
...
im,b,e,rs='http'+(s_ssl?'s':'')+'://metrics.apple.com/b/ss/'+un+'/1/G.9p1/'
...
q='r';else if(t=='charSet'){q='ce';if(s_wd[k]&&s_em==2)s_wd[k]='UTF-8'
...
o.onclick.toString():'';if(oc.indexOf("s_gs(")>=0)return ''}ta=o.target;

/* Begin Modified Code for Relative Exit Links */
/*h=o.href?o.href:'';*/
var a_l=s_wd.location;
